...
const props = defineProps({
  occupationDetail: Object,
});

const toast = useToast();
const form = useForm({
  id: props.occupationDetail.id,
  name: props.occupationDetail.occupation_name,
  workContent: props.occupationDetail.work_contents.map((item) => {
    item.workContentName = item.work_content_name;
    return item;
  }),
  redirectOption: null,
});
...
const addWorkContent = () => {
  form.workContent.push({ workContentName: null, id: null });
};
...
const submit = (e) => {
  ...
  form.put(route("admin.master.occupation.update"), {
    onSuccess: () => {
...
<template>
  ...
  <AdminLayout title="職種管理">
    <MainContent>
      ...
      <MasterEditBox @emitSubmit="submit" link="occupation" :data="occupationDetail">
