...
class TableManagementController extends Controller
{
    ...
    public function exportAttendData(MonthRequest $request)
    {
        ...
        $map_data = $break_data->map(function ($item) use ($month_date) {
            ...
            $users_dakou_data = $user_data->map(function ($item) use ($month_date) {
                ...
                foreach ($dakoku_by_user as $item) {
                    $real_work_times += $item['work_time_seconds'] - $item['rest_seconds'];
                }
                $real_work_times = Helper::secondsToTime($real_work_times);
                ...
                foreach ($dakou_data as $key => $val) {
                    if ($val->dp_other_flg) {
                        ...
                        if (mb_strpos($other_flg_txt, "残業") !== false) {
                            $other_count = str_replace('H 残業', '', $other_flg_txt);
                    ...
                    if ($val->dp_type == '1日') {
                        ...
                        $attend_date_nums += 0.5;
                    }
                }
                $over_time = Helper::secondsToTime($over_time);
                // 日曜出勤
                $sunday_attend_count = count($dakou_data->filter(function ($item) {
                    return date('w', strtotime(($item->target_date))) == 0;
                }));
                // ・運転手手当 (打刻で運転を選択した回数)
                ...
                return [
                    'user_name'     => $user_name,
                    'workdays'      => $attend_date_nums,
                    'worktimes'     => $real_work_times,
                    'over_time'     => $over_time,
        ...
        foreach ($map_data as $item) {
            ...
            $sheet1->mergeCells('C' . ($row + 4) . ':H' . ($row + 4));
            $sheet1->getRowDimension($row + 4)->setRowHeight(25);
            $sheet1->getStyle('B' . ($row + 4) . ':H' . ($row + 4))->applyFromArray($this->sheet_style);
            // writing header of user table
            $sheet1->setCellValue('B' . $row + 5, '月給者');
            if (strpos($item['work_pattern'], 'pa') !== false) {
                $sheet1->setCellValue('C' . $row + 5, '出勤時間');
            } else {
                $sheet1->setCellValue('C' . $row + 5, '出勤日数');
            }
            $sheet1->setCellValue('D' . $row + 5, '平日休日残業時間');
            $sheet1->setCellValue('E' . $row + 5, '日曜出勤');
            $sheet1->setCellValue('F' . $row + 5, '運転手手当');
            $sheet1->setCellValue('G' . $row + 5, '遠距離手当');
            $sheet1->setCellValue('H' . $row + 5, '有給');
            $sheet1->getStyle('B' . ($row + 5) . ':H' . ($row + 5))->applyFromArray($this->sheet_style);
            $sheet1->getRowDimension($row + 5)->setRowHeight(25);

            foreach ($item['data'] as $key => $val) {
                $sheet1->setCellValue('B' . $row + 6, $val['user_name']);
                if (strpos($item['work_pattern'], 'pa') !== false) {
                    $sheet1->setCellValue('C' . $row + 6, $val['worktimes']);
                } else {
                    $sheet1->setCellValue('C' . $row + 6, $val['workdays']);
                }
                $sheet1->setCellValue('D' . $row + 6, $val['over_time']);
                $sheet1->setCellValue('E' . $row + 6, $val['sunday_count']);
                $sheet1->setCellValue('F' . $row + 6, $val['drive_count']);
                $sheet1->setCellValue('G' . $row + 6, $val['undrive_count']);
                $sheet1->setCellValue('H' . $row + 6, $val['paid_count']);
                $sheet1->getStyle('B' . ($row + 6) . ':H' . ($row + 6))->applyFromArray($this->sheet_style);
    ...
    public function exportAttendanceBook(MonthRequest $request)
    {
        ...
        foreach ($users as $keys => $user) {
            foreach ($user['location'] as $key => $location) {
                // $count = DakouChild::with('dakoku')->whereHas('dakoku', function ($q) use ($users, $user, $location, $year, $month) {
                //     $q->where('dp_user', $user['user_id']);
                //     $q->whereYear('target_date', $year);
                //     $q->whereMonth('target_date', $month);
                // })->where('dp_work_location_id', $location['id'])
                // ->count();
                $count = DakouData::with(['dakoku_children'])->where('dp_user', $user['user_id'])
                    ->whereYear('target_date', $year)
                    ->whereMonth('target_date', $month)
                    ->whereHas('dakoku_children', function ($q) use ($location) {
                        $q->where('dp_work_location_id', $location['id']);
                    })->count();
                $row_data[$keys]['user_name'] = $user['user_name'];
                $row_data[$keys]['locations'][$key] = $count;
            }
        }
        if (count($row_data) == 0) {
            return response()->json(['success' => false, 'message' => 'データはありません。'], 200);
        ...
        if ($nums_of_locations > 12) {
            $sheet->mergeCellsByColumnAndRow(3, 3, $nums_of_locations + 2, 3); // start col, start row, end col, end row
            $sheet->mergeCellsByColumnAndRow(1, 2, $nums_of_locations + 3, 2);
            // 最後の列は合計 // $nums_of_locations+4, not 3
            // ユーザー別合計列のマージとデータの書き込み
            $sheet->mergeCellsByColumnAndRow($nums_of_locations + 3, 3, $nums_of_locations + 3, 4);
            $sheet->setCellValueByColumnAndRow($nums_of_locations + 3, 3, '合計');
            $sheet->getColumnDimensionByColumn($nums_of_locations + 3)->setWidth(6);
        } else {
            $sheet->mergeCellsByColumnAndRow(3, 3, 14, 3); // 14 N column
            $sheet->mergeCellsByColumnAndRow(1, 2, 15, 2); // 15 O column
        ...
        foreach ($locations as $key => $location) {
            ...
            // adjust cell width
            $sheet->getColumnDimensionByColumn($key + 3)->setWidth($location_column_width);
        ...
        foreach ($row_data as $row_key => $user) {
            $sheet->setCellValue('A' . $row_key + 5, $row_key + 1);
            $sheet->setCellValue('B' . $row_key + 5, $user['user_name']);
            $sheet->getRowDimension($row_key + 5)->setRowHeight(-1);
            ...
            // ユーザー別合計
            if ($nums_of_locations > 12) {
                $sheet->setCellValueByColumnAndRow($nums_of_locations + 3,  $row_key + 5,  array_sum($user['locations']));
            } else {
                $sheet->setCellValue('O' . $row_key + 5,  array_sum($user['locations']));
            }
        }

        // 現場別合計
        $sheet->mergeCells('A' . (count($row_data) + 5) . ':B' . (count($row_data) + 5));
        $sheet->setCellValue('A' . count($row_data) + 5, '合計');

        $sheet->fromArray([$sum_by_location_arr], null, 'C' . count($row_data) + 5);
        $sheet->setCellValueByColumnAndRow($nums_of_locations + 3, count($row_data) + 5, array_sum($sum_by_location_arr));

        // スタイル適用 A2 ~ 
        $sheet->getStyleByColumnAndRow(1, 2, $nums_of_locations + 3, count($row_data) + 5)->applyFromArray($this->sheet_style);
    ...
    public function exportAttendPerLocationData(Request $request)
    {
        ...
        foreach ($users as $key => $user) {
            ...
            $i = 1;
            while ($i <= $max_day) {
                $dakoku_data = DakouData::with('dakoku_children.work_location')->where('dp_user', $user->id)
                    ...
                    ->whereHas('dakoku_children.work_location', function ($query) use ($location) {
                        $query->where('dp_work_location_id', $location);
                    })->exists();
                if ($dakoku_data) {
    ...
    public function exportManPowerData(Request $request)
    {
        ...
        $max_day = date('t', strtotime($month_date));
        $rows = $work_locations->collect()->map(function ($item) use ($max_day, $year, $month) {
        ...
        $c_col_txt = ['大工', '解体', '加工'];
        // 現場が2つ以上ならA5～AH7スタイルで行追加。
        // 最後の行のボーダー厚を厚い線にするために2つの現場名を初期セット
        for ($i = 0; $i < count($rows); $i++) {
            if (count($rows) > $i + 2) {
                $sheet->insertNewRowBefore($i * 3 + 8, 3);
                $sheet->mergeCells('A8:A10');
            }
        }
        foreach ($rows as $r_key => $row) {
            $sheet->setCellValue('A' . $r_key * 3 + 5, $row['location']);
            foreach ($row['cols'] as $sub_key => $sub_row) {
                $sheet->setCellValue('B' . $r_key * 3 + $sub_key + 5, $c_col_txt[$sub_key]);
                $sheet->setCellValue('C' . $r_key * 3 + $sub_key + 5, array_sum($sub_row));
                foreach ($sub_row as $col_key => $val) {
                    $sheet->setCellValue($this->date_set_cols[$col_key - 1] . $r_key * 3 + $sub_key + 5, $val);
                }
            }

            $style = $sheet->getStyle('A' . $r_key * 3 + 5);
            $font  = $style->getFont();
            $font->setSize(16);
        ...
        // 祝日設定
        $last_rows = count($rows) > 1 ? count($rows) * 3 + 4 : 10;
