...
onMounted(() => {
    if (props.occupation) {
        occupationData.value = props.occupation.data.map((item) => {
...
const toastBack = ref();
const removeData = () => {
    deleteForm.delete(route('admin.master.occupation.destroy'), {
        onSuccess: () => {
            ...
            occupationData.value = props.occupation.data.filter(filter => filter.id != deleteForm.id)
        },
        onErrorr: () => {
...
const filterAction = () => {
    form.get(route('admin.master.occupation.index'));
}
</script>
<template>
    ...
    <AdminLayout title="職種管理 ">
        <MainContent>
            ...
            <MasterContentBox link="occupation" @filter="(e) => e == 'detail' ? detailFilter = true : detailFilter = false">
                ...
                <div class="w-full border datatable center">
                    <DataTable :value="occupationData" data-key="id" selectionMode="multiple" class="p-datatable-sm">
                        ...
                        <Column header="操作">
                            <template #body="slotProps">
                                <div class="flex items-center justify-center gap-3">
                                    <Link :href="route('admin.master.occupation.show', { id: slotProps.data.id })">
                                    <FontAwesomeIcon icon="fa-solid fa-eye" class="text-sky-500" />
                                    </Link>
                                    <Link :href="route('admin.master.occupation.edit', { id: slotProps.data.id })">
        ...
        <div class="flex items-center justify-center px-6 mt-6">
            <LinkPagination :data="occupation" />
        </div>
