...
<template>
            ...
            <ContentBox title="新規登録">
                <template #header>
                    <div class="w-full flex items-center justify-between">
                        ...
                        <div class="btn_group flex items-center gap-col-1">
                ...
                <div class="w-full user-register-page pb-8">
                    <form @submit.prevent="submit" class="input-form">

                        <div class="mt-8 form-inner">
                            <div class="label-field label-right mt-2">
                                ...
                                <div class="w-full">
                                    <div class="flex items-center gap-2">
                                        <div class="w-full p-input-icon-left">
                                            ...
                                            <InputText v-model="form.code" class="w-full" :class="duplicateStatus ? 'p-invalid' : ''"
                                                placeholder="識別子用のコードを入力してください。" @change="duplicateCodeCheck"/>
                                        </div>
                        ...
                        <div class="mt-8 form-inner center">
                            ...
                            <div class="input-field">
                                <div class="w-full p-input-icon-left">
                                    ...
                                    <InputText v-model="form.name" class="w-full" :class="{'p-invalid':form.errors.name}" placeholder="ユーザー名を入力してください。" />
                                </div>
                        ...
                        <div class="mt-8 form-inner center">
                            ...
                            <div class="input-field">
                                <div class="w-full p-input-icon-left">
                                    <i class="pi pi-at"></i>
                                    <InputText v-model="form.email" class="w-full" :class="{'p-invalid':form.errors.email}" :autoComplete="false" placeholder="メールアドレスを入力してください。" />
                                </div>
                        ...
                        <div class="mt-8 form-inner">
                            ...
                            <div class="input-field flex items-center gap-2">
                                ...
                                <Button label="サーチ" icon="pi pi-search" size="small" class="shrink-0 py-2.5 w-32" severity="info"
                                    @click="visibleWorkStyleDialog = !visibleWorkStyleDialog" />
                            </div>
                        </div>
                        ...
                        <hr>
                        <div class="security-setting mt-4">
                            <div class="mt-8 form-inner center">
                                <div class="label-field label-right">
                                    <InputLabel value="パスワード自動生成" />
                                </div>
                                <div class="input-field flex items-center gap-8">
                                    ...
                                    <div v-if="form.password && autoGenPassword" class="ml-8 relative">
                        ...
                        <div class="mt-8 text-center">
                            <Button type="submit" :label="!form.processing ? '保存する' : ''"
                                :icon="form.processing ? 'pi pi-spin pi-spinner' : 'pi pi-save'" severity="success"
                                class="w-44 m-auto" :disabled="form.processing" />
                        </div>
                    </form>
                </div>
            </ContentBox>
    ...
    <Dialog v-model:visible="visibleWorkStyleDialog" header="サーチ : 休憩時間・勤務形態管理" modal dismissableMask :draggable="false"
        ...
        <div class="filter-tab flex items-center gap-2 w-full border-b pb-2">
        ...
        <div class="data-list datatable center">
            <DataTable :value="workParentCodeList" v-model:selection="selectionWorkStyle" selectionMode="single"
                data-key="id" stripedRows class="p-datatable-sm border border-teal-300">
        ...
        <div class="selected-item mt-4 flex items-center w-full gap-8">
            <label for="" class="whitespace-nowrap">選択</label>
            <div class="w-full h-14 flex items-center border p-2 rounded-md">
                <div v-if="selectionWorkStyle"
                    class="relative flex items-center gap-2 bg-sky-600 text-white p-2 rounded-md">
                    <p>
                        {{ selectionWorkStyle?.break_work_pattern_cd }}
                        {{ selectionWorkStyle?.organization.organization_name }}
                        {{ selectionWorkStyle?.break_name }}
                    </p>
                    <i class="pi pi-times cursor-pointer" @click="selectionWorkStyle = null"></i>
                </div>
            </div>
        </div>
        <template #footer>
