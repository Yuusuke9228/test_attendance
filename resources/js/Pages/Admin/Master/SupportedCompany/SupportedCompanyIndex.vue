...
onMounted(() => {
	if (props.supportedCompany) {
		supportedCompanyData.value = props.supportedCompany.data.map((item) => {
...
const toastBack = ref();
const removeData = () => {
	deleteForm.delete(route('admin.master.supported_company.destroy'), {
		onSuccess: () => {
			...
			supportedCompanyData.value = props.supportedCompany.data.filter(filter => filter.id !== deleteForm.id)
		},
		onErrorr: () => {
...
const filterAction = () => {
	form.get(route('admin.master.supported_company.index'));
}
</script>
<template>
	...
	<AdminLayout title="応援に行く先の会社">
		<MainContent>
			...
			<MasterContentBox link="supported_company"
				...
				<div class="w-full border datatable center">
					<DataTable :value="supportedCompanyData" data-key="id" selectionMode="multiple" class="p-datatable-sm">
						...
						<Column header="操作">
							<template #body="slotProps">
								<div class="flex items-center justify-center gap-3">
									<Link :href="route('admin.master.supported_company.show', { id: slotProps.data.id })">
									<FontAwesomeIcon icon="fa-solid fa-eye" class="text-sky-500" />
									</Link>
									<Link :href="route('admin.master.supported_company.edit', { id: slotProps.data.id })">
		...
		<div class="flex items-center justify-center px-6 mt-6">
			<LinkPagination :data="supportedCompany" />
		</div>
